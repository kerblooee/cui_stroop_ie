...
expName = 'cui_stroop'  # from the Builder filename that created this script
expInfo = {'participant': ''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\resha\\Documents\\psychopy_exps\\cui_stroop_ie\\cui_stroop_ie_lastrun.py',
...
instr1 = visual.TextStim(win=win, name='instr1',
    text='default text',
    font='Arial',
    pos=[0, 0], height=.06, wrapWidth=1.2, ori=0, 
...
word_rect = visual.Rect(
    win=win, name='word_rect',
    width=(1, .4)[0], height=(1, .4)[1],
    ori=0, pos=(0, 0),
...
block_instr = visual.TextStim(win=win, name='block_instr',
    ...
    languageStyle='LTR',
    depth=0.0);
block_nr = visual.TextStim(win=win, name='block_nr',
    text='default text',
    font='Arial',
    pos=(0, .1), height=.2, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=-1.0);
...
for thisPractice in practice:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    background.setFillColor(str(bgColor))
    background.setLineColor(str(bgColor))
    word.setText(str(text))
    resp.keys = []
    resp.rt = []
    _resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [word_rect, background, word, resp, mask]
    ...
    while continueRoutine:
        ...
        # *word_rect* updates
        if word_rect.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
            word_rect.frameNStart = frameN  # exact frame index
            word_rect.tStart = t  # local t and not account for scr refresh
            word_rect.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(word_rect, 'tStartRefresh')  # time at next scr refresh
            word_rect.setAutoDraw(True)
        
        # *background* updates
        if background.status == NOT_STARTED and frameN >= 60:
            # keep track of start time/frame for later
        ...
        # *word* updates
        if word.status == NOT_STARTED and frameN >= 62:
            # keep track of start time/frame for later
        ...
        # *resp* updates
        waitOnFlip = False
        if resp.status == NOT_STARTED and frameN >= 64:
            # keep track of start time/frame for later
        ...
        # *mask* updates
        if mask.status == NOT_STARTED and frameN >= 64:
            # keep track of start time/frame for later
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    practice.addData('word_rect.started', word_rect.tStartRefresh)
    practice.addData('word_rect.stopped', word_rect.tStopRefresh)
    practice.addData('background.started', background.tStartRefresh)
    practice.addData('background.stopped', background.tStopRefresh)
    ...
    practice.addData('mask.started', mask.tStartRefresh)
    practice.addData('mask.stopped', mask.tStopRefresh)
    ...
    # keep track of which components have finished
    feedbackComponents = [feedback_2, reminder]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if feedback_2.status == STARTED:
            ...
            if tThisFlipGlobal > feedback_2.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if reminder.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > reminder.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
    ...
    practice.addData('reminder.started', reminder.tStartRefresh)
    practice.addData('reminder.stopped', reminder.tStopRefresh)
    thisExp.nextEntry()
...
blocks = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('blockNrs.csv'),
...
for thisBlock in blocks:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    block_nr.setText(str(block))
    ready_2.keys = []
    ready_2.rt = []
    _ready_2_allKeys = []
    # keep track of which components have finished
    instr_blockComponents = [block_instr, block_nr, ready_2]
    for thisComponent in instr_blockComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    instr_blockClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "instr_block"-------
    while continueRoutine:
        # get current time
        t = instr_blockClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=instr_blockClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        ...
        # *block_nr* updates
        if block_nr.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            block_nr.frameNStart = frameN  # exact frame index
            block_nr.tStart = t  # local t and not account for scr refresh
            block_nr.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(block_nr, 'tStartRefresh')  # time at next scr refresh
    ...
    blocks.addData('block_nr.started', block_nr.tStartRefresh)
    blocks.addData('block_nr.stopped', block_nr.tStopRefresh)
    # check responses
    if ready_2.keys in ['', [], None]:  # No response was made
        ready_2.keys = None
    blocks.addData('ready_2.keys',ready_2.keys)
    if ready_2.keys != None:  # we had a response
        blocks.addData('ready_2.rt', ready_2.rt)
    blocks.addData('ready_2.started', ready_2.tStartRefresh)
    blocks.addData('ready_2.stopped', ready_2.tStopRefresh)
    ...
    for thisTrial in trials:
        ...
        for thisComponent in trialComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        trials.addData('word_rect.started', word_rect.tStartRefresh)
        trials.addData('word_rect.stopped', word_rect.tStopRefresh)
